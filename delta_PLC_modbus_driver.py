...
class PLCDelta(ModbusClient):
    ...
    def __init__(self, ip_addr='127.0.0.1', port=502):
        ModbusClient.__init__(self, host=ip_addr,
                              port=port,
                              auto_open=True,
                              auto_close=False)
    ...
    def write_plc_bit(self, bit_addr: str, bit_value: bool):
        if bit_addr[0].upper() in 'MY':
            self.write_single_coil(bit_addr=self.convert_addr(bit_addr),
                                   bit_value=bit_value)

    def read_plc_bit(self, bit_addr: str):
        if bit_addr[1].upper() == 'X':
            return self.read_discrete_inputs(self.convert_addr(bit_addr))[0]
        else:
            return self.read_coils(self.convert_addr(bit_addr))[0]

    def read_plc_register(self, reg_addr: str):
        if reg_addr[0].upper() == 'X':
            return self.read_input_registers(self.convert_addr(reg_addr))[0]
        elif reg_addr[0].upper() in 'YD':
            return self.read_holding_registers(self.convert_addr(reg_addr))[0]

    def write_plc_register(self, reg_addr: str, reg_value: int):
        if reg_addr[0].upper() in 'YD':
            self.write_single_register(reg_addr=self.convert_addr(reg_addr),
                                       reg_value=reg_value)

    def write_plc_registers(self, list_of_values: list,
                            reg_addr: str,
                            n: int = 1):
        ...
        for value in list_of_values[:n]:
            self.write_plc_register(reg_addr=reg_addr, reg_value=value)
            reg_addr = reg_addr[0] + str(int(reg_addr[1:]) + 1)


def main():
    plc = PLCDelta(ip_addr='localhost', port=10002)
    while True:
